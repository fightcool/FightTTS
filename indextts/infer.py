...
class IndexTTS:
    ...
    def bucket_segments(self, segments, bucket_max_size=4) -> List[List[Dict]]:
        ...
        # 过滤掉空的segments
        outputs = [o for o in outputs if o["len"] > 0]
        
        # 如果所有segments都是空的，返回空列表
        if not outputs:
            print(">> Warning: All segments are empty, returning empty bucket list")
            return []
    ...
    def pad_tokens_cat(self, tokens: List[torch.Tensor]) -> torch.Tensor:
        # 检查空列表，防止torch.cat()错误
        if not tokens:
            raise ValueError("pad_tokens_cat: tokens list is empty")
    ...
    def infer_fast(self, audio_prompt, text, output_path, verbose=False, max_text_tokens_per_segment=100,
        ...
        # 检查是否有空的segments
        if not all_segments:
            print(">> Warning: No valid segments found, returning empty audio")
            return torch.zeros(1, 1), sampling_rate
        ...
        for item_tokens in all_text_tokens:
            ...
            # 检查空的token列表
            if batch_num == 0:
                print(">> Warning: Empty token list found, skipping...")
                continue
            if batch_num > 1:
        ...
        for items in chunk_latents:
            ...
            # 调试日志：检查items是否为空
            if not items:
                print(f">> ERROR: Empty items in chunk_latents! Skipping...")
                continue
            print(f">> DEBUG: Processing {len(items)} items in chunk")
        ...
        # 修复：当wavs为空时返回空音频而不是抛出错误
        if not wavs:
            print(f">> WARNING: No wavs generated, returning empty audio")
            # 返回一个空的音频张量，形状为 [1, 0]
            wav = torch.zeros((1, 0), dtype=torch.float32)
        else:
            print(f">> DEBUG: Concatenating {len(wavs)} wav tensors")
    ...
    def infer(self, audio_prompt, text, output_path, verbose=False, max_text_tokens_per_segment=120,
        ...
        # 修复：当wavs为空时返回空音频而不是抛出错误
        if not wavs:
            print(f">> WARNING: No wavs generated in infer method, returning empty audio")
            # 返回一个空的音频张量，形状为 [1, 0]
            wav = torch.zeros((1, 0), dtype=torch.float32)
        else:
            print(f">> DEBUG: Concatenating {len(wavs)} wav tensors in infer method")
            wav = torch.cat(wavs, dim=1)
        wav_length = wav.shape[-1] / sampling_rate
        print(f">> Reference audio length: {cond_mel_frame * 256 / sampling_rate:.2f} seconds")
        print(f">> gpt_gen_time: {gpt_gen_time:.2f} seconds")
        ...
        if output_path:
            ...
            torchaudio.save(output_path, wav.type(torch.int16), sampling_rate)
            print(">> wav file saved to:", output_path)
            self._set_gr_progress(1.0, "audio generation completed!")
            return output_path
        else:
            # 返回音频数据
            wav_data = wav.type(torch.int16)
